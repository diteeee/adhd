module.exports = (io) => {
  ...
  router.post('/', async (req, res) => {
      ...
      try {
          ...
          const mesazhi = "You left a review!";

            const notification = await Notification.create({
                notificationUserID: reviewUserID,
                mesazhi,
            });

          io.to(reviewUserID).emit("newNotification", notification);
          await review.save();
  ...
  router.delete('/:reviewID', auth, async (req, res) => {
    ...
    try {
      ...
      // Only allow if user owns review or is admin
      ...
      const mesazhi = "Your review was deleted.";

      const notification = await Notification.create({
        notificationUserID: review.reviewUserID,
        mesazhi,
      });

      // Emit real-time notification to the user who owned the review
      io.to(review.reviewUserID).emit("newNotification", notification);
